...
TEST_F(BindingsTest, tombstoneAndNewAttrSameName)
{
    // This tests that a tombstone followed by a real value in the same layer
    // doesn't happen (would be a logic error), but if we add both to the same
    // layer, the sort should handle it. Actually, let's test the expected use
    // case: layering a real value on top of a tombstone layer.
